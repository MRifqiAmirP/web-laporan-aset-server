...
<body style="max-width: 100%; overflow-x: hidden;">
    ...
    $query = "SELECT tb_pegawai.*, tb_req_edit_server.* FROM tb_pegawai INNER JOIN tb_req_edit_server ON tb_pegawai.NO_TICKET COLLATE utf8mb4_general_ci = tb_req_edit_server.NO_TICKET";
    ...
    <div id="main" class="row" style="height: inherit;">
        <div class="sidebar col-md-2" style="height: 100%;">
            ...
            <a href="" style="background-color: #004080; color: #00ffff; text-shadow: 0 0 5px #00ffff; border-left: 4px solid #00ffff;"><i class="fas fa-edit" style="margin-right: 10px;"></i>Request Edit Server</a>
                    ...
                    <div class="card content-card" style="width: 18rem; margin-right: 20px; margin-top: 20px;">
                        <div class="card-body" style="display: flex; flex-direction: column; justify-content: space-between;">
                            <div>
                                ...
                                <h6 class="card-subtitle mb-2 text-muted"><?= $result['NO_TICKET']; ?></h6>
                                <p class="card-text">Nama: <?= $result['NAMA_PEGAWAI']; ?></p>
                                <p class="card-text" style="margin-top: -20px;">No. Pegawai: <?= $result['NO_PEG']; ?></p>
                                <p class="card-text" style="margin-top: -20px;">Username: <?= $result['USERNAME']; ?></p>
                                <p class="card-text" style="margin-top: -20px;">Email: <?= $result['EMAIL_ADDRESS']; ?></p>
                                <p class="card-text" style="margin-top: -20px;">Posisi: <?= $result['POSISI']; ?></p>
                            </div>
    ...
    while ($result = mysqli_fetch_array($sql)) :
        $sqlTbServer = mysqli_query($conn, "SELECT * FROM tb_server WHERE ID_SERVER = {$result['ID_SERVER']}");
        $resultTbServer = mysqli_fetch_array($sqlTbServer);
        ...
        <div id="detail<?= $indexBtn; ?>" class="detail container-fluid">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p><i class="fas fa-user"></i>Nama: <?= $result['NAMA_PEGAWAI']; ?></p>
                    <p><i class="fa-sharp-duotone fa-solid fa-id-card"></i>No. Pegawai: <?= $result['NO_PEG']; ?></p>
                    <p><i class="fas fa-user-circle"></i>Username: <?= $result['USERNAME']; ?></p>
                    <p><i class="fas fa-envelope"></i>Email: <?= $result['EMAIL_ADDRESS']; ?></p>
                    <p><i class="fas fa-briefcase"></i>Posisi: <?= $result['POSISI']; ?></p>
                </div>
                <div class="col-md-6">
                    <p><i class="fas fa-memory"></i>Hostname: <?= $resultTbServer['HOSTNAME']; ?></p>
                    <p><i class="fas fa-memory"></i>ID Server: <?= $result['ID_SERVER']; ?></p>
                    <p><i class="fas fa-memory"></i>Total Memori: <?= $result['TOTAL_MEMORI']; ?> Gb ⬅ (<?= $resultTbServer['TOTAL_MEMORI']; ?> Gb)</p>
                    <p><i class="fas fa-microchip"></i>Total RAM: <?= $result['TOTAL_RAM']; ?> Gb ⬅ (<?= $resultTbServer['TOTAL_RAM']; ?> Gb)</p>
                    <p><i class="fas fa-microchip"></i>Total CPU: <?= $result['TOTAL_CPU']; ?> Cores ⬅ (<?= $resultTbServer['TOTAL_CPU']; ?> Cores)</p>
                </div>
            </div>
            <?php if ($resultAvp[1] != 'Accept') { ?>
                <div class="btn-container">
                    <form action="../php/proses.php" method="post">
                        ...
                        <button data-index="<?= $indexBtn; ?>" type="submit" name="respon" value="accept_edit" class="btn-accept">Accept</button>
                    </form>
                    <form action="../php/proses.php" method="post">
                        <input id="ticket<?= $indexBtn; ?>" type="hidden" name="reject" value="<?= $result[0]; ?>">
                        <button data-index="<?= $indexBtn; ?>" type="submit" name="respon" value="reject_edit" class="btn-reject">Reject</button>
                    </form>
    ...
    <script>
        document.querySelectorAll('.btn-accept').forEach(accept => {
            accept.addEventListener('click', function() {
                ...
                ajax.send('action=accept_edit&ticket=' + ticket);
            });
        });
        ...
        document.querySelectorAll('.btn-reject').forEach(reject => {
            reject.addEventListener('click', function() {
                ...
                ajax.send('action=reject_edit&ticket=' + ticket);
            });
        });
    </script>
